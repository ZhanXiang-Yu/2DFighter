...
ifeq ($(config),debug_x64)
  ...
  Fighter_config = debug_x64

else ifeq ($(config),debug_x86)
  raylib_config = debug_x86
  Fighter_config = debug_x86

else ifeq ($(config),debug_arm64)
  raylib_config = debug_arm64
  Fighter_config = debug_arm64

else ifeq ($(config),release_x64)
  raylib_config = release_x64
  Fighter_config = release_x64

else ifeq ($(config),release_x86)
  raylib_config = release_x86
  Fighter_config = release_x86

else ifeq ($(config),release_arm64)
  raylib_config = release_arm64
  Fighter_config = release_arm64

else
  $(error "invalid configuration $(config)")
endif

PROJECTS := raylib Fighter
...
Fighter: raylib
ifneq (,$(Fighter_config))
	@echo "==== Building Fighter ($(Fighter_config)) ===="
	@${MAKE} --no-print-directory -C game -f Makefile config=$(Fighter_config)
endif
